...
serve(async (req) => {
  ...
  try {
    // Require authenticated user (verify_jwt = false in config; we validate manually)
    ...
    const limited = await enforceRateLimit(supabaseAdmin, userId, 'elevenlabs-tts', 30, 60);
    if (limited) {
      return new Response(
