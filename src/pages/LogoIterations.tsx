...
const RoseVariant = ({ size = 188 }: { size?: number }) => {
  ...
  const idHalo = `${uid}-rose-halo`;
  const idPetal = `${uid}-rose-petal`;
  const idCore = `${uid}-rose-core`;
  const PETAL_COUNT = 8;
  ...
  return (
    <svg viewBox="0 0 240 240" width={size} height={size} role="img" aria-label="Cathedral Rose">
      <defs>
        ...
        <linearGradient id={idPetal} x1="50%" y1="0%" x2="50%" y2="100%">
      ...
      {/* Core medallion */}
      <circle cx="120" cy="120" r="36" fill={`url(#${idCore})`} />
...
const CrossVariant = ({ size = 188 }: { size?: number }) => {
  const uid = useId().replace(/:/g, '');
  const idHalo = `${uid}-cross-halo`;
  const idFill = `${uid}-cross-fill`;
  const idGlow = `${uid}-cross-glow`;
  ...
  return (
    <svg viewBox="0 0 240 240" width={size} height={size} role="img" aria-label="Pilgrim's Cross">
      <defs>
        ...
        <linearGradient id={idFill} x1="0%" y1="0%" x2="0%" y2="100%">
      ...
      {/* Outer halo */}
      <circle cx="120" cy="120" r="116" fill={`url(#${idHalo})`} />
