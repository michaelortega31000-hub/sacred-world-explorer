...
const PlaceCategoryFilter = ({
  ...
  return <ToggleGroup type="single" value={value} onValueChange={handleChange} className={`bg-card/80 backdrop-blur-sm border border-border/50 rounded-lg p-1 ${className}`}>
      <ToggleGroupItem value="all" aria-label="Tous les lieux" className="gap-1.5 px-3 py-2 data-[state=on]:bg-primary data-[state=on]:text-primary-foreground rounded-md text-sm font-medium">
        
        <span className="hidden sm:inline">Tous</span>
      </ToggleGroupItem>
      
      <ToggleGroupItem value="religious_site" aria-label="Lieux religieux" className="gap-1.5 px-3 py-2 data-[state=on]:bg-primary data-[state=on]:text-primary-foreground rounded-md text-sm font-medium">
